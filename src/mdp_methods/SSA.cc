...
float mdp_planner::compute_Q_value(Policy_Node* Node,Policy_Node* root, const MDP_Net::Ptr& pNet)
{
  //std::cout<<"Im here at mdp_planner"<<std::endl;
  std::vector<Policy_Node*> queue;
  Policy_Node *temp;
  float state_value = 0;
  Vec2 v_app = mdp_state_t::getActionVector(Node->optimal_act);
  if(Node->parent==NULL)
  {
    return 100;
  }
...
void mdp_planner::update_policy(Policy_Node* root, std::vector<Policy_Node*> queue_node,const MDP_Net::Ptr& pNet)
{
  for(int i=0; i<queue_node.size();i++)
  {
    ...
    for(int j=0;j<8;j++)
    {
      ...
      float state_value_temp = mdp_planner::compute_Q_value(queue_node[i],root,pNet);
...
void mdp_planner::SSA(Policy_Node* root, std::vector<Policy_Node*> queue_node, const MDP_Net::Ptr& pNet)
{
  ...
  while(error>0.1)
  {
    ...
    for(int i=0;i<queue_node.size();i++)
    {
      float state_value = mdp_planner::compute_Q_value(queue_node[i], root, pNet);
      float error_temp = state_value-queue_node[i]->state_value;
      queue_node[i]->state_value = state_value;
      if(error_temp>error)
      {
        error = error_temp;
...
void mdp_planner::GMSPI(mdp_state_t* goal, const MDP_Net::Ptr& pNet)
{
  ...
  mdp_planner::SSA(root,old_queue,pNet);
  int flag = 1;
  while(flag)
  {
    flag=0;
    ...
    mdp_planner::SSA(root,new_queue,pNet);
    mdp_planner::update_policy(root, new_queue,pNet);
    old_queue.clear();
