...
void SSP::initTransMatrix() {
  ...
  //std::cout<<"IM here in initial matrix"<<std::endl;
  ...
  mdp_state_t *state = pNet->mdp_states[i];
    ...
    if(flag == 0){
      //std::cout<<"Im here in fill matrix"<<std::endl;
        fillTransMatrix(transMatrix, state->id, pNet->n_cols, state->probs,i);
    }
    //f_ij.push_back(0);
  }
...
void SSP::fillTransMatrix(SpMat& matrix, const int &state_id, const int &cols, const vector<double>& probs,int oo) {
  //cout << "I am here fill Trans 1: " << matrix.size() << " " << state_id << " " << cols << endl;
  //std::cout<<"Im here in fillin matrix"<<std::endl;
  double obst_prob = 0.000001;
  int num = 1;
  double prob_sum = 0;
  int flag = 0;
